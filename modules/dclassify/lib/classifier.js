  ...
  train: function (trainingSet) {
    ...
    for (i = 0; i < categories.length; i++) {
      ...
      if (this.categoryCounts[category]) {
        this.categoryCounts[category] = this.categoryCounts[category] + subSet.length;
      } else{
      ...
      for (j = 0; j < subSet.length; j++) {
        ...
        for (k = 0; k < tokenlist.length; k++) {
          ...
          if (!this.cattokenCounts[category]) {
            this.cattokenCounts[category] = {};
          }
          if (!this.cattokenCounts[category][token]) {
            this.cattokenCounts[category][token] = 1;
          } else {
            this.cattokenCounts[category][token] = 1 + this.cattokenCounts[category][token];
          }
        }
      }
    }
    ...
    for (i = 0; i < categories.length; i++) {
      ...
      for (k in this.tokens) {
        if (this.tokens.hasOwnProperty(k)) {
          ...
          var total =  this.cattokenCounts[category][k] ? this.cattokenCounts[category][k] : 0;
          var percentage = 0;
          if (total === 0) {
            percentage = 0;
          } else {
